...
class SmolVLMDataReader(IterableDataset):
    ...
    def _iter_one_dataset(self, dataset_name: str) -> Iterable[dict]:
        ...
        n = len(meta["datalist"])
        traj_indices = list(range(n))
        if self.training:
            # If every entry has a 'weight' field, do weighted sampling
            weights = None
            if n > 0 and all("weight" in d for d in meta["datalist"]):
                weights = [float(d["weight"]) for d in meta["datalist"]]
                total = sum(weights)
                if total > 0:
                    weights = [w / total for w in weights]
            if weights is not None:
                traj_indices = random.choices(
                    range(n), weights=weights, k=n)
            else:
