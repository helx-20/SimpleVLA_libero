...
def linear_warmup_only(step, start, warmup, base_lr):
    """Linear warmup without cosine decay — LR stays at base_lr after warmup."""
    if step < start:
        return 0.0
    if warmup <= 0:
        return base_lr
...
def update_group_lrs(optim, step, args):
    ...
    Warmup is *always* applied when ``warmup_steps > 0``, regardless of
    ``use_cosine_decay``.  The ``use_cosine_decay`` flag only controls
    whether the LR is further decayed via a cosine schedule after warmup.
    """
    base = {
    ...
    if step < args.freeze_steps:
        # Freeze period: only action heads are trained (at full LR — warmup
        # is deliberately skipped for the heads during this phase so they
        # can adapt quickly while the backbone is frozen).
        ...
        for name, base_lr in base.items():
            # Always apply warmup first (if configured).
            wm_lr = linear_warmup_only(
                step, args.freeze_steps, args.warmup_steps, base_lr,
            )
            if args.use_cosine_decay:
                # Cosine decay on top of the post-warmup LR.
                progress = step - args.freeze_steps - args.warmup_steps
                total_decay = max(1, args.iters - args.freeze_steps - args.warmup_steps)
                if progress > 0 and total_decay > 0:
                    ratio = 0.5 * (1.0 + math.cos(
                        math.pi * min(1.0, progress / total_decay)))
                    decay = args.min_lr_ratio + (1.0 - args.min_lr_ratio) * ratio
                else:
                    decay = 1.0
                new_lr = wm_lr * decay
            else:
