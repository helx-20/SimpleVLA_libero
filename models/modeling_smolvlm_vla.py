...
class SmolVLMVLA(PreTrainedModel):
    ...
    def __init__(self, config: SmolVLMVLAConfig, *args, **kwargs):
        ...
        # ---- Offline-PPO heads ----
        # Lazily used: present in state_dict so checkpoints round-trip, but the
        # flow-matching forward() path doesn't touch them. Old SimpleVLA
        # checkpoints without these keys load fine (HF's from_pretrained
        # initializes missing tensors).
        init_logstd = float(getattr(config, "init_logstd", -1.0))
        value_hidden = int(getattr(config, "value_hidden", 512))
        self.actor_logstd = nn.Parameter(
            torch.full((self.num_actions, dim_action), init_logstd, dtype=torch.float32)
        )
        self.value_head = nn.Sequential(
            nn.LayerNorm(vlm_hidden_size),
            nn.Linear(vlm_hidden_size, value_hidden),
            nn.GELU(),
    ...
    def forward(
        ...
        action: torch.Tensor = None,        # [B, T=num_actions, D=dim_action]
        mode: str = "flow_match",
        steps: int = 10,
    ) -> Dict[str, torch.Tensor]:
        """Dispatch on ``mode``:

        * ``"flow_match"`` (default) — the original SimpleVLA training loss.
        * ``"policy"``                — ``policy_forward`` output, but routed
          through ``forward`` so DDP's forward hook wraps it. Returns
          ``{"mean_normalized": ..., "value": ...}`` and, if ``action`` was
          passed, additionally ``"new_log_prob"`` — computed here (not by the
          caller) so that ``actor_logstd``'s usage lives inside ``forward``
          and DDP's Reducer tracks it. Calling ``self.gaussian_log_prob(...)``
          externally on a DDP-wrapped module triggers "marked ready twice".

        The PPO trainer must use ``mode="policy"`` (via ``model(...)``, never
        ``model.policy_forward(...)`` directly), otherwise DDP's Reducer
        doesn't see the iteration and grad all-reduce becomes unreliable.
        """
        if mode == "policy":
            mean_norm, value = self.policy_forward(
                input_ids=input_ids,
            ...
            out = {"mean_normalized": mean_norm, "value": value}
            if action is not None:
                # Compute log_prob under (mean_norm, actor_logstd) inside
                # forward() so DDP sees actor_logstd get used this iteration.
                out["new_log_prob"] = self.gaussian_log_prob(action, mean_norm)
            return out
        # else: flow-matching training path (unchanged)
        assert action is not None, "flow_match mode requires an `action` tensor"
        """
    ...
    # ============================ Offline PPO helpers ============================
    # Approach: wrap the deterministic flow-matching head with a diagonal
    # Gaussian. The flow-matching integration output is treated as the policy
    # mean μ (normalized action space), and a = μ + σ·ε with σ = exp(logstd).
    #
    # log_prob is exact under this Gaussian assumption. The bias is small
    # because the pre-trained flow-matching head is well-localized — multiple
    # noise rollouts produce nearly the same mean — but we never claim it's
    # the true policy density. Good enough for offline PPO's clipped surrogate
    # used as a *regularizer* on top of BC.
    ...
    def _flow_match_mean(
        self,
        vlm_features: torch.Tensor,
        proprio_norm: torch.Tensor,
        steps: int,
    ) -> torch.Tensor:
        """Euler-integrate the velocity field to produce a normalized-action mean.

        Returns ``μ`` of shape ``(B, num_actions, dim_action)``. Caller controls
        whether grads flow through the transformer.
        """
        B = proprio_norm.shape[0]
        D = self.action_space.dim_action
        device = proprio_norm.device
        dtype = proprio_norm.dtype
    ...
    def policy_forward(
        ...
        """Differentiable: returns ``(mean_normalized, value)``.

        Used by the PPO training loop to recompute log_prob/value on stored
        actions. The action transformer + VLM both see gradients here; pass
        ``self.vlm.requires_grad_(False)`` from the trainer to freeze the
        backbone (recommended for offline PPO).
        """
        enc = self.forward_vlm_efficient(image_input, image_mask, input_ids)
        vlm_features = enc["vlm_features"]                  # (B, T_vlm, D)

        proprio_norm = self._normalize_proprio(proprio)
        mean_norm = self._flow_match_mean(vlm_features, proprio_norm, steps)
        value = self.value_head(vlm_features.mean(dim=1)).squeeze(-1)
        return mean_norm, value
    ...
    def act_and_value(
        ...
        """No-grad rollout helper. Returns the dict the PPO buffer wants.

        Keys:
          mean_normalized : (B, T_action, D) — policy mean in normalized space
          action_normalized : (B, T_action, D) — sampled (or μ if deterministic)
          action : (B, T_action, D) — postprocessed for env consumption
          log_prob : (B,) — Gaussian log_prob of the *normalized* action
          value : (B,)
          logstd : (T_action, D) — actor_logstd snapshot
        """
        self.eval()
        mean_norm, value = self.policy_forward(
            input_ids, image_input, image_mask, proprio, steps=steps,
        )
        logstd = self.actor_logstd
        if deterministic:
            sample_norm = mean_norm
        else:
            noise = torch.randn_like(mean_norm)
            sample_norm = mean_norm + torch.exp(logstd) * noise
        log_prob = self.gaussian_log_prob(sample_norm, mean_norm)
        action = self.action_space.postprocess(sample_norm)
        return {
            "mean_normalized": mean_norm,
            "action_normalized": sample_norm,
            "action": action,
            "log_prob": log_prob,
            "value": value,
            "logstd": logstd.detach().clone(),
        }

    def gaussian_log_prob(
        self,
        action_normalized: torch.Tensor,    # (B, T_action, D)
        mean_normalized: torch.Tensor,      # (B, T_action, D)
    ) -> torch.Tensor:
        """log N(a; μ, diag(σ²)) summed over (T_action, D). Returns (B,)."""
        # Force fp32: under bf16 autocast, exp(2 * -3) = 0.0025 and diff²/var
        # can overflow the 7-bit mantissa. Log_prob is small enough to compute
        # in fp32 without a memory hit.
        logstd = self.actor_logstd.float()                  # (T_action, D)
        a = action_normalized.float()
        m = mean_normalized.float()
        var = torch.exp(2.0 * logstd)
        diff_sq = (a - m) ** 2 / var
        logp = -0.5 * diff_sq - logstd - 0.5 * self.LOG_2PI
        return logp.sum(dim=(-1, -2))                       # (B,)

    def entropy(self) -> torch.Tensor:
        """Per-decision entropy of the diagonal Gaussian (scalar)."""
        return (self.actor_logstd + 0.5 * (self.LOG_2PI + 1.0)).sum()
